...
import { ArrowLeft, Search, MessageSquare, HelpCircle, Users, ThumbsUp, MessageCircle, Eye, Clock, PenTool } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-8">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 border border-[#e5e7eb] rounded-lg focus:outline-none focus:border-[#1344FF] transition-colors bg-white shadow-sm"
          />
        </div>
        <button className="px-6 py-3 bg-[#1344FF] text-white rounded-lg hover:bg-[#0d34cc] transition-all shadow-md font-medium flex items-center justify-center gap-2">
      ...
      <div className="bg-white rounded-xl shadow-md overflow-hidden">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    {type === 'qna' && post.isAnswered && (
                      <span className="px-2 py-0.5 bg-green-100 text-green-700 text-xs rounded-full font-medium">답변완료</span>
                    )}
                    {type === 'qna' && !post.isAnswered && (
                      <span className="px-2 py-0.5 bg-gray-100 text-gray-600 text-xs rounded-full font-medium">답변대기</span>
                    )}
                    {type === 'mate' && (
        ...
        <div className="p-6 border-t border-gray-100 flex justify-center gap-2">
          <button className="w-8 h-8 rounded-full bg-[#1344FF] text-white flex items-center justify-center font-bold">1</button>
          <button className="w-8 h-8 rounded-full hover:bg-gray-100 text-[#666666] flex items-center justify-center font-medium">2</button>
          <button className="w-8 h-8 rounded-full hover:bg-gray-100 text-[#666666] flex items-center justify-center font-medium">3</button>
          <button className="w-8 h-8 rounded-full hover:bg-gray-100 text-[#666666] flex items-center justify-center font-medium">...</button>
        </div>
      </div>
    </div>
  );
